...
# Subset to MYH7 objects
myh7_all <- gencode_gr[which(gencode_gr$gene_id == "ENST00000355349.4")]
myh7_all_df <- myh7_all %>% as.data.frame()
myh7_t_all <- myh7_all[which(myh7_all$transcript_id == "ENST00000355349.4")]
myh7_t <- myh7_t_all[which(myh7_t_all$type == "transcript")]
myh7_t_df <- myh7_t %>% as.data.frame()
```
...
plot_reads <- function(reads, xlim_gr) {
  ...
  autoplot(myh7b_reads, geom = "area", stat = "coverage") + 
    xlim(xlim_gr) + 
...
sampled_files <- lapply(bam_files_per_condition, sample, size = 1)
sampled_files
myh7_reads <- lapply(sampled_files, 
                      readGAlignments, 
                      param = ScanBamParam(which = myh7b_t),
                      use.names = TRUE)
...
plot_reads <- function(reads, xlim_gr) {
  # Plotting function from ggbio for GenomicAlignments objects
  autoplot(myh7_reads, geom = "area", stat = "coverage") + 
    xlim(xlim_gr) + 
    scale_y_continuous(expand = c(0,0)) + 
    scale_x_continuous(expand = c(0,0)) +
    theme_paperwhite() +
...
myh7_coverage_plots <- lapply(myh7_reads, plot_reads, xlim_gr = myh7_t)
```
...
# Prepare transcript model
myh7_transcript <- crunch(gencode_txdb, which = myh7_gene)
colnames(values(myh7b_transcript))[4] <- "model"
myh7_transcript <- split(myh7_transcript, myh7_transcript$tx_id)


# Plot the transcript model
myh7_transcript_plot <- autoplot(myh7_transcript, 
                                  aes(type = model), 
                                  label = FALSE) + theme_paperwhite() +
  theme(panel.border = element_blank(),
        axis.text.x=element_blank(),
...
# Prepare ideogram 
ideo_plot <- plotIdeogram(genome = "hg38", which = myh7_t)
...
tracks(ideo_plot,
       NF = myh7_coverage_plots[["NF"]], 
       MYH7B = myh7_transcript_plot, 
       heights = c(1, 2, 2, 2, 0.6),
       label.bg.fill = "#ffffff",
       label.bg.color = "#ffffff") 
ggsave("figures/MYH7_whole_gene_tracks.tiff", height = 5, width = 10)

```

```{r}
# Let's figure out which exon we need to plot
show(myh7_transcript_plot)
myh7_exons <- myh7_t_all[which(myh7_t_all$type == "exon")]
myh7_exons_df <- myh7_exons %>% as.data.frame()
# I think it's exon 8
myh7_exons_df[8,]
myh7_skipped_exon <- myh7_exons[which(myh7_exons$exon_id == "ENSE00000860157.1")]
# Let's add some padding on either side 100 bp either side
myh7b_se_padded <- resize(myh7b_skipped_exon, width = width(myh7b_skipped_exon)+50*2, fix = "center")
myh7b_7_padded <- resize(myh7b_exons[7], width = width(myh7b_skipped_exon)+50*2, fix = "center")
myh7b_9_padded <- resize(myh7b_exons[9], width = width(myh7b_skipped_exon)+50*2, fix = "center")
